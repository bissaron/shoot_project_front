...
export default {
  ...
  methods: {
    ...
    async renderChart() {
      ...
      const months = Object.keys(this.usageData);
      const days = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']; // ใช้วันในสัปดาห์ที่แน่นอน
      const counts = Array.from({ length: months.length }, () => Array(days.length).fill(0));
      const colors = [
  ...
  'rgba(128, 0, 128, 0.2)'   // วันเสาร์
      ...
      months.forEach((month, i) => {
        days.forEach((day, j) => {
          counts[i][j] = this.usageData[month][day] || 0;
        });
      });
      ...
      this.chart = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: days.map((day, i) => ({
            ...
            data: counts.map((monthCounts) => monthCounts[i]),
            backgroundColor: colors[i], // เลือกสีตามวัน
