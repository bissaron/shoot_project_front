  ...
  methods: {
    async getAllGun() {
      try {
        ...
        const loggedInCid = auth ? auth.c_id : null;
        ...
        if (response.status == 200) {
          // กรองข้อมูลการจองเฉพาะ c_id ของผู้ใช้ที่ลงชื่อเข้าใช้
          this.gunItems = response.data.filter(item => item.customer.c_id === loggedInCid);
        }
      } catch (err) {
        console.error(err);
      }
